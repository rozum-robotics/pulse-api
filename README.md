...
while True:
    try:
        ...
        robot.set_pose(start_pose, speed=SPEED)
        robot.await_motion()

        robot.set_position(position_target, speed=SPEED)
        robot.await_motion()

        # command the robot to go through multiple position waypoints
        # (execute a trajectory)
        robot.run_positions(position_targets, speed=SPEED)
        robot.await_motion()
        ...
        robot.run_poses(pose_targets, SPEED, tcp_max_velocity=TCP_VELOCITY_1CM)
